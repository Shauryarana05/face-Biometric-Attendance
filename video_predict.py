...
# def run_face_rec():
#     print("Waiting to ensure images are saved")
#     time.sleep(2)  # Give 2 seconds to complete image saving
#     subprocess.Popen(['python', 'face_rec.py'])
...
if __name__ == "__main__":
    ...
    frame_count=1  
    temp_frame=None     #new line

    #line 110-113 is created by me to save the temporary picture in a folder 

    output_folder = "Temporary Picture"
                ...
                if label == 0:
                    if score > args.threshold:
                        ...
                        if frame_count<=10:   
                            ...
                            filename = os.path.join(output_folder, f"temp_pic{frame_count}.jpg")        #this line is saving the photo in a folder Temporary Picture
                            cv2.imwrite(filename, temp_frame)
                            print(f"image {frame_count} saved")
                            frame_count+=1
                        elif int(current_time)%56==0 and frame_count>10:
                            frame_count=1
